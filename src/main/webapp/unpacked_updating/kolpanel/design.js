...
/*
//引入realPath
import realPath0 from './realPath.js';
//引入template(直接执行)
...
function listQuestion(){
    ...
    //施工中-start
    let filtered = filterActive(questions.sort(compare('index')));
...
function listParse(json){
    ...
    window.sets = json.setting ? JSON.parse(json.setting)[0] : (new CreateSetting());
    window.sets.allocations.question_list = window.questions;
//    window.style = json.style;
    ...
    for (let i = 0,l = list.length; i < l; i++){
        ...
        drawQuestion(item.type,item.form,item.col,item)
    }

    window.questions.sort(compare('itemId'));

    //for (let i = 0, l = window.questions.length; i < l; i++){
    //    window.questions[i].itemId = Number(i) + 1
    //}

    //for (let i in list){
    //
    //    list[i].itemId = Number(i) + 1;
    //
    //}
    //
    //
