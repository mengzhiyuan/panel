...
$(function(){
        ...
        follow_options: [
            {
                folId: '',
                folName: '会员查询',
                folAddress: './html/kol/membership.html',
                folLogo: 'fa fa-address-book',
                folEmail: '',
                selected: false
            },{
                folId:'',
                folName: '会员添加',
                folAddress: './html/kol/dataDetail.html',
                folLogo: 'fa fa-user-plus',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '匹配',
                folAddress: './html/kol/match.html',
                folLogo: 'fa fa-exchange',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '会员管理',
                folAddress: './html/kol/operationalCenter.html',
                folLogo: 'fa fa-book',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '单位查询',
                folAddress: './html/kol/membershipHospital.html',
                folLogo: 'fa fa-institution',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '单位添加',
                folAddress: './html/kol/dataHospital.html',
                folLogo: 'fa fa-plus-square',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '单位管理',
                folAddress: './html/kol/operationHospital.html',
                folLogo: 'fa fa-reorder',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '项目管理',
                folAddress: './html/kol/projectManage.html',
                folLogo: 'fa fa-tasks',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '添加项目',
                folAddress: './html/kol/newProject.html',
                folLogo: 'fa fa-puzzle-piece',
                folEmail: '',
                selected: false
            },{
                folId: '',
                folName: '修改审核',
                folAddress: './html/kol/updateExamine.html',
                folLogo: 'fa fa-question-circle',
                folEmail: '',
                selected: false
            },{
                folId:'',
                folName: '导出审核',
                folAddress: './html/kol/exportExamine.html',
                folLogo: 'fa fa-question-circle-o',
                folEmail: '',
                selected: false
            }
        ],
    ...
    const methods = {
        ...
        submitFilter: function(){
            ...
            window.open('./html/kol/statistics.html')
        },
        ...
        save_follows: function(){
            ...
            for (let i = 0, follows = self.follow_options; i < follows.length; i++){
                if (follows[i].selected){
                    let this_follow = JSON.parse(JSON.stringify(follows[i]));
                    this_follow.folEmail = email;
                    delete(this_follow.selected);
                    to_send.follows.push(this_follow);
                    selected.push(this_follow)
                }
            }
            $.ajax({
                url: realPath + '/management/followAddOrUpdate',
                type: 'post',
...
//                self.selected[data[i].catName] = true;
        ...
        //修改审核-----------------------------------------------------------------------------
        if (JSON.parse(sessionStorage.getItem('sysUser')).email === 'Zero.Liu@jfcmc.com'){
            $('#update_examine').css('display', 'block')
        }

        //我的关注-start-----------------------------------------------------------------------
        $.ajax({
            url: realPath + '/management/followList',
            type: 'post',
            contentType: 'application/json;charset=utf-8',
            dataType: 'json',
            data: JSON.stringify({
            ...
            success: function(d){
                if (d[0].result){
                    for (let i = 0, data = d[0].result; i < data.length; i++){
                        for (let j = 0, follow = self.follow_options; j < follow.length; j++){
                            if (data[i].folName === follow[j].folName){
                                self.followed.push(self.follow_options[i])
                            }
                        }
                    }
                } else {
                    simpleAlert('请设置关注', '#f56c6c')
                }
            },
            error: function(e){
                simpleAlert('操作失败', '#f56c6c')
