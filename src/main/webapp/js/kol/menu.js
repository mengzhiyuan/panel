/**
 * Created by wangxiangyang on 2018/5/9.
 */
...
$(function(){
        ...
        function listMenu(list){
            ...
            for (i = 0; i < len; i++){
                ...
                if (list[i]){
                    ...
                    if (list[i].menuName === '系统管理'){//单独处理设置菜单
                        ...
                        continue;
                    } else if (list[i].menuName === '帮助') {
                        if (list[i].subAuthorityList.length) {
                            let sub_item = $('.dropdown-usermenu'),
                                sub = list[i].subAuthorityList,
                                l = sub.length,
                                j;
                            for (j = 0; j < l; j++) {
                                if (sub[j]) {
                                    ...
                                    sub_item.find('li')[1].innerHTML = '<a href="'+ sub[j].dataUrl +'">帮助</a>'
                                }
                            }
                        }
                        continue
            ...
            ul.append($(`<li><a href="//www.kolpanel.net/jfcpanel/html/login.html?target=sky-oa"><i class="fa fa-bar-chart-o"></i> Sky-OA </a>
                  </li>
                  <li>
    ...
    $('.profile_pic img')
	    ...
	    var str = test.split("/");
	    if(str[str.length-1]=="main_kangpai"){
	    	top.location.href ="./html/kol/edit-portrait.html"
	    }else{
	    	top.location.href ="./edit-portrait.html"
	    }
	    //window.open('//www.kolpanel.net/jfcpanel/html/kol/edit-portrait.html')
	});
    let user_message = JSON.parse(sessionStorage.getItem('sysUser')),
        img_src = user_message.url,
        email = user_message.email,
        menu_node = $('#menu1'),
        bool_main = document.location.href.match(/\/main_kangpai/),
        jump_target_export = '//www.kolpanel.net/jfcpanel/html/kol/exportExamine.html',
        jump_target_update = '//www.kolpanel.net/jfcpanel/html/kol/updateExamine.html',
        link_export = `<li>
                                    <a href="`+ jump_target_export +`">
