...
const survey_template = {
    ...
    choice: {
        template: `
                    <div class="item" v-if="correspond && config.exist">
                        <div class="item-head">
                            ...
                            <div class="item-options" v-else>
                                <p class="option" v-for="option in options" :style="{ width: config.layout.width_percent }">
                                    <label :for="'item-' + id + 'option-' + option.option_id">
                                        <input class="radio" type="radio" :id="'item-' + id + 'option-' + option.option_id" :name="'item-' + id" :checked="option.spotting" @change="changeChoice(option.option_id)">
                ...
                handler: function (value, old) {
                    for (let i = 0; i < this.options.length; i++) {
                        if (parseInt(value.option_id) === this.options[i].option_id) {
                            ...
                            value.option_index = answer_option.option_index;
                            value.option_code = answer_option.option_code;
                            value.option_content = answer_option.option_content;
                            this.options[i].spotting = true;
    ...
    check: {
                ...
                <div class="item" v-if="correspond && config.exist">
                    ...
                    <div class="item-chest">
                        <div class="item-options">
                            <p class="option" v-for="option in options" :style="{ width: config.layout.width_percent }">
                                <label :for="'item-' + id + 'option-' + option.option_id">
                                    <input class="checkbox" type="checkbox" :id="'item-' + id + 'option-' + option.option_id" :name="'item-' + id" :checked="option.spotting" @change="changeCheck($event,option.option_id)">
                                    <span>{{ option.option_content }}</span>
                                    <input class="blank" type="text" v-if="option.blank" v-model="option.option_addition">
                                </label>
                            </p>
            ...
            changeCheck: function (event, option_id) {
                ...
                if ($(event.currentTarget).prop('checked')) {
                    ...
                    let obj = {
                        ...
                        option_content: answer_option.option_content
                    };
                    if (answer_option.blank) {
                    ...
                    if (this.answer[0].option_id !== '') {
                        this.$set(this.answer, this.answer.length, obj);
                        // this.answer.push(obj)
                    } else {
                        this.$set(this.answer, 0, obj);
                        // this.answer[0] = obj
                    }
                } else {
        ...
        created: function () {
            for (let i = 0; i < this.options.length; i++) {
                ...
                //     if (this.options[i].option_id === this.answer[k].option_id){
                //         spotting = true;
                //         break
                //     }
                // }
                this.options[i].spotting = spotting;
            }
        },
        mounted: function () {
            let condition = this.config.random_option;
                ...
                handler: function (value, old) {
                    ...
                    for (let i = 0; i < this.options.length; i++) {
                        ...
                        for (let k = 0; k < value.length; k++) {
                            if (this.options[i].option_id === value[k].option_id) {
                                f = true;
                            }
                            // this.options[i].spotting = this.options[i].option_id === value[k].option_id
                        }
                        this.options[i].spotting = f;
                    }
                    this.$forceUpdate();
                },
                deep: true
    ...
    blank: {
        ...
        created: function () {
            if (this.config.the_class === 'multi') {
                for (let i = 0; i < this.sub_questions.length; i++) {
                    if (this.answer[i]) {
                ...
                this.answer[0].question_id = this.sub_questions[0].question_id;
                this.answer[0].question_index = this.sub_questions[0].question_index;
                this.answer[0].question_content = this.sub_questions[0].question_content;
                                ...
                                <tbody>
                                    <tr v-for="option in options">
                                        ...
                                        <td v-for="question in sub_questions">
                                            <span v-if="question.the_class==='matrix_choice'"><input :name="'item-' + id + 'sub_question-' + question.question_id" :q_id="question.question_id" :o_id="option.option_id" type="radio" @change="changeChoice($event,question.question_id,option.option_id)"></span>
                                            <span v-if="question.the_class==='matrix_check'"><input :name="'item-' + id + 'sub_question-' + question.question_id" :q_id="question.question_id" :o_id="option.option_id" type="checkbox" @change="changeCheck($event,question.question_id,option.option_id)"></span>
                                            <span v-if="question.the_class==='matrix_blank'"><input :name="'item-' + id + 'sub_question-' + question.question_id" :q_id="question.question_id" :o_id="option.option_id" type="text" @change="changeBlank($event,question.question_id,option.option_id)"></span>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
        ...
        created: function () {
            ...
            for (let i = 0; i < this.sub_questions.length; i++) {
                let about_question = {
                    ...
                    form: this.sub_questions[i].the_class,
                    options: []
                };
                for (let j = 0; j < this.options.length; j++) {
                    ...
                    to_push.option_checked = about_question.class === 'matrix_blank';

                    about_question.options.push(to_push);
                }
                        ...
                        if (value[i].the_class === 'matrix_blank') {
                            ...
                            for (let j = 0; j < value[i].options.length; j++) {
                                if (!value[i].options[j].option_answer) {
                                    ...
                                    $(this.$el).find('[q_id=' + q_id + '][o_id=' + value[i].options[j].option_id + ']').val(value[i].options[j].option_answer);
                                }
                            }
                        } else {
                            sub_pass = false;
                            for (let j = 0; j < value[i].options.length; j++) {
                                if (value[i].options[j].option_checked) {
                                    ...
                                    $(this.$el).find('[q_id=' + q_id + '][o_id=' + value[i].options[j].option_id + ']').prop('checked', value[i].options[j].option_checked);
                                }
                            }
                        }
                        if (!sub_pass) {
    ...
    upload: {
        template: `
                <div class="item item-blank" v-if="correspond && config.exist">
                    <div class="present">
                        <div class="item-head">
                            <i class="index"><b class="present">{{ number }}</b></i>
                            <p class="item-name">{{ name }}</p>
    ...
    geolocation: {
        template: `
                <div class="item item-geolocation" v-if="correspond && config.exist">
    ...
    order: {
        template: `
                <div class="item item-order" v-if="correspond && config.exist">
                    <div class="present">
                        <div class="item-head">
                            <i class="index"><b class="present">{{ number }}</b></i>
                            <p class="item-name">{{ name }}</p>
    ...
    linkage: {
        template: `
                <div class="item item-linkage" v-if="correspond && config.exist">
                    <div class="present">
                        <div class="item-head">
                            <i class="index"><b class="present">{{ number }}</b></i>
                            <p class="item-name">{{ name }}</p>
    ...
    sex: {
        template: `
                <div class="item item-choice" v-if="correspond && config.exist">
                    <div class="present">
                        <div class="item-head">
                            <i class="index"><b class="present">{{ number }}</b></i>
                            <p class="item-name">{{ name }}</p>
...
/* harmony import */ var _common_alerting__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../common/alerting */ "./js/swing/common/alerting.js");
...
const survey = new Vue({
    ...
    methods: {
            ...
            if (could_complete) {
                ...
                for (let i = 0; i < self.answers_object.answers_now.length; i++) {
                    answers_final.push({
                        ...
                        index: self.answers_object.answers_now[i].index,
                        form: self.answers_object.answers_now[i].the_class,
                        answer_content: self.answers_object.answers_now[i]
                    });
                    delete self.answers_object.answers_now[i].type;
                    delete self.answers_object.answers_now[i].index;
                ...
                $.ajax({
                    ...
                    success: function (d) {
                        ...
                        if (!self.answers_object.respondent) {
                            self.answers_object.respondent = d[0].result;
                        }
                    },
                    error: function (e) {}
                });
            } else {
                _common_alerting__WEBPACK_IMPORTED_MODULE_1__["default"].alertAntoRemove('请完成此页所有题目后再提交', '#f56c6c');
            }
        },
        ...
        getData: function (page, method) {
            ...
            if (method === 'jump') {
                url_clip = '/survey/page_next_answer';
            } else {
                url_clip = '/survey/page_answer';
                to_send.url = document.location.href;
            }

            $.ajax({
                url: this.realPath + url_clip,
                type: 'post',
                dataType: 'json',
                contentType: 'application/json;charset=utf-8',
            ...
            //             "answer": [
            //                 {
            //                     "ansContent": "[{\"option_id\":0,\"option_index\":1,\"option_code\":\"\",\"option_content\":\"第一项\",\"option_addition\":\"\",\"sub_id\":162,\"ans_id\":\"\"}]",
            //                     "ansId": 1,
            //                     "ansIndex": "1",
            //                     "ansOptionAddition": "",
            //                     "ansOptionAnswer": "",
            ...
            //                     "subBreak": "1",
            //                     "subCollected": "",
            //                     "subConfig": "[{\"exist\":true,\"page_now\":2,\"required\":true,\"neck\":false,\"func_display\":false,\"random_option\":\"fixed\",\"layout\":{\"orientation\":\"portrait\",\"cols_number\":1,\"width_percent\":\"100%\"},\"recommend\":{\"bool\":false,\"recommended\":\"\"},\"page\":1,\"break\":1}]",
            //                     "subId": 162,
            //                     "subIndex": "1",
            //                     "subLogic": [
            //
            ...
            //                     "subName": "duoxuian多选",
            //                     "subNumber": "2",
            //                     "subOption": "[{\"option_id\":0,\"option_index\":1,\"option_code\":\"\",\"option_content\":\"第一项\",\"checked\":false,\"mutex\":false,\"blank\":false},{\"option_id\":2,\"option_index\":2,\"option_code\":\"\",\"option_content\":\"第二项\",\"checked\":false,\"mutex\":false,\"blank\":false},{\"option_id\":3,\"option_index\":2,\"option_code\":\"\",\"option_content\":\"第三项\"},{\"option_id\":3,\"option_index\":3,\"option_code\":\"\",\"option_content\":\"第四项\"}]",
            //                     "subPId": 1,
            //                     "subPage": "1",
            //                     "subQuestions": [
            //
        ...
        initialAnswer: function (arr) {
            let ar = arr || [];
            for (let i = 0; i < ar.length; i++) {

                let this_answer = ar[i];

                for (let j = 0; j < this.questions.length; j++) {
                    let the_answer = this.questions[j].answer;
                    if (this_answer.subId === the_answer.sub_id) {

                        let container = {};

                        if (['check', 'blank', 'matrix', 'sort'].indexOf(the_answer.type) > -1) {
                            container = JSON.parse(this_answer.ansContent);
                        } else {
                            container = JSON.parse(this_answer.ansContent)[0];
                        }

                        container.ans_id = this_answer.ansId;
                        container.type = this_answer.ansType;

                        for (let k in container) {

                            if (typeof container[k].option_id !== "undefined") {
                                Vue.set(the_answer, k, container[k]);
                            } else {
                                the_answer[k] = container[k];
                            }
                        }
                    }
                }
        ...
        renderItem: function (data) {
            ...
            if (data.subConfig) {
                ...
                if (data.config.the_class) {
                    data.answer.the_class = config.the_class;
                }
        ...
        submitAll: function () {
            this.clearPage();
            sessionStorage.setItem('status', 'C');
            document.location.href = './swing/finish.html';
        }
    },
    mounted: function () {
        let self = this;
