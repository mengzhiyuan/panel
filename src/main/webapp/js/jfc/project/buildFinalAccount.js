...
const buildBudget = {
    ...
    budgetMethods: {//vue方法对象
        ...
        getSurplus: function(arr){

            let budget = arr.budget ? parseFloat(arr.budget) : 0,
                supplementary = arr.supplementaryBudget,
                finalAccount = arr.finalAccount ? parseFloat(arr.finalAccount) : 0;

            for (let i = 0; i < supplementary.length; i++){
                budget += parseFloat(supplementary[i].supplementary)
            }
            arr.surplus = budget - finalAccount;
        },
            ...
            watch: {
                budgets: {
                    handler: function(val,old){
                        ...
                        this.sum.finalAccount = this.getSum(this.budgets, 'finalAccount');
                        for (let k = 0; k < this.budgets.length; k++){
                            this.getSurplus(this.budgets[k])
                        }

                        this.sum.surplus = this.getSum(this.budgets, 'surplus');
                ...
                sum: {
                    handler: function(val,old){
                        ...
                        this.average.finalAccount = this.sample.finalAccount > 0 ? this.sum.finalAccount / this.sample.finalAccount : '';
                        this.average.surplus = this.sample.surplus > 0 ? this.sum.surplus / this.sample.surplus : '';
                        ...
                        $('#final_cost').val(this.sum.finalAccount)
                    },
                    deep: true
                },
                sample: {
