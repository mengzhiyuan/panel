...
$(function() {
    ...
    var table = $('#about_budget');
    table.find('.kind').prop('disabled', true);
    table.find('.add_row').prop('disabled', true);
    table.find('.add_col').prop('disabled', true);
    table.find('.budget').prop('disabled', true);
    table.find('.abstract').prop('disabled', true);
    table.find('.supplementary').prop('disabled', true);
    table.find('.surplus').prop('disabled', true);
    table.find('.addGoods').prop('disabled', true);
    table.find('.removeGoods').prop('disabled', true);
    //table.find('.remark1').prop('disabled', true);
    //table.find('.remark2').prop('disabled', true);

    function budgetBookSearch(){
        var to_send = {};
        to_send.proId = sessionStorage.getItem("proId");
        $.ajax({
            url : "" + realPath + "/project/finalAccountsingle",
            type : "post",
            dataType : "json",
            async : true,
            ...
            success : function(data) {
                ...
                if (data[0].budgetInfo && data[0].budgetInfo[0]['infoId']){
                ...
                var list = data[0].proList[0],
                    ...
                    final = data[0].finalAccounts[0];

                if (final){
                    sessionStorage.setItem('accId', final ? final.accId : '')
                }
                ...
                //决算相关
                if (final){
                    $('#info_actual_money').val(final.accActualAmount);
                    $('#final_cost').val(final.accFinalCost);
                    $('#gross_profit').val(final.accGrossProfit);
                    $('#bonus_ratio').val(final.accBonusRatio);
                    $('#sell_bonus').val(final.accSalesBonus);
                    $('#project_bonus').val(final.accProjectBonus);
                }
    ...
    function submitAll(event){
        ...
        */
        ...
        let data = {
            ...
            //},
            info_actual_money: $('#info_actual_money').val(),
            acc_gross_profit: $('#gross_profit').val(),
            acc_sales_bonus: $('#sell_bonus').val(),
            acc_final_cost: $('#final_cost').val(),
            acc_project_bonus: $('#project_bonus').val(),
            acc_bonus_ratio: $('#bonus_ratio').val(),
            proId:sessionStorage.getItem("proId") || '',
            accId: sessionStorage.getItem('accId') || ''
            //infoId: sessionStorage.getItem('infoId') || ''
        };
        $.ajax({
            url: "" + realPath + "/project/finalAccountsAdd",
            type: 'post',
            contentType: 'application/json;charset=utf-8',
            dataType: 'json',
            async: true,
            ...
            success: function(d){
                document.location.href = './finalAccountList.html'
            },
            error: function(e){
                console.log(e);
                alert('保存失败。')
