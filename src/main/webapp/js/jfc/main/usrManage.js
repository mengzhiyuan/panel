...
var TableInit = function() {
	...
	function responseHandler(res) {
		...
		var roles_node = document.getElementById("role_update"),
            department_node = document.getElementById("department_update");
		roles_node.innerHTML = '<option value="">请选择</option>';
		department_node.innerHTML = '<option value="">请选择</option>';
		  for (var i = 0, len = role.length; i < len; i++){
	            var opt = document.createElement("option");
	            opt.value = role[i].id;
	            opt.innerText = role[i].roleValue;
	            roles_node.appendChild(opt);
	        }
	        ...
	        for (i = 0, len = department.length; i < len; i++){
	            ...
	            department_node.appendChild(opt);
	        }
