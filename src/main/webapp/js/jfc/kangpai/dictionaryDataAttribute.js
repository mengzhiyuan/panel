...
$(function(){
    ...
    //    $.ajax({
    //        url: realPath + '/dictionary/listAttribute',
    //        type: 'post',
    //        dataType: 'json',
    //        async: true,
    ...
    function initWeb(){
        ...
        var TableInit = function() {
            ...
            function deleteItem(row){
                ...
                $.ajax({
                    url: realPath + '/dictionary/deleteAttribute',//路径待更正
                    type: 'post',
                    dataType: 'json',
                    async: true,
            ...
            oTableInit.Init = function () {
                $('#list').bootstrapTable({
                    url: realPath + '/dictionary/listAttribute',         // 请求后台的URL（*）
                    method: 'post',                      // 请求方式（*）
                    ...
                    onEditableSave: function (field, row, oldValue, $el) {
                        $.ajax({
                            type: "post",
                            url: realPath + '/dictionary/updateAttribute',
                            contentType: 'application/json;charset=utf-8',
                            data: JSON.stringify({
                                attriId: row.id,
                                attribute: [row.name]
                    ...
                    responseHandler: function(data){
                        var attributes = data[0]['attribute'],//键名待更正
                        ...
                        for (i = 0; i < len; i++){
                            data.rows.push({
                                ...
                                'id': attributes[i].attriId,
                                'name': attributes[i].attriName
                            });
                            data.total += 1
                        }
                        return data;
        ...
        $('#add').on('click', function(){
            ...
            function addSave(){
                ...
                $.ajax({
                    url: realPath + '/dictionary/insertAttribute',
                    type: 'post',
                    dataType: 'json',
    ...
    function displayList(data){
        ...
        for (i = 0; i < len; i++){
            str += '<li class="items" dataId="' + attributes[i]["attriId"] + '"><span class="itemValue">' + attributes[i]["attriName"] + '</span><span class="btns"><span class="extra_btn edit_btn"></span><span class="extra_btn delete_btn"></span></span></li>'
        }
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (res > 0){
                        ...
                        $('.okoButton').one('click', function(event){
                            listRequest()
                        })
                    } else {
