...
$(function(){
    ...
    function initWeb() {
        ...
        var TableInit = function () {
            ...
            function deleteItem(row) {
                ...
                $.ajax({
                    url: realPath + '/dictionary/natureDelete',//路径待更正
                    type: 'post',
                    dataType: 'json',
                    async: true,
            ...
            oTableInit.Init = function () {
                $('#list').bootstrapTable({
                    url: realPath + '/dictionary/natureSearch',         // 请求后台的URL（*）
                    method: 'post',                      // 请求方式（*）
                    ...
                    onEditableSave: function (field, row, oldValue, $el) {
                        $.ajax({
                            type: "post",
                            url: realPath + '/dictionary/natureUpdate',
                            contentType: 'application/json;charset=utf-8',
                            data: JSON.stringify({
                                natId: row.id,
                                natName: row.name
                    ...
                    responseHandler: function (data) {
                        var nature = data[0]['nature'],//键名待更正
                            len = nature.length,
                            i;
                        data.total = 0;
                        data.rows = [];
        ...
        $('#add').on('click', function () {
            ...
            function addSave() {
                ...
                $.ajax({
                    url: realPath + '/dictionary/natureInsert',
                    type: 'post',
                    dataType: 'json',
    ...
    function listRequest(){
        $.ajax({
            url: realPath + '/dictionary/natureSearch',
            type: 'post',
            dataType: 'json',
            async: true,
            success: function(data){
                displayList(data[0]['nature'])
            },
            error: function(d1,d2,d3){
                console.log(d1);
                console.log(d2);
    ...
    function displayList(data){
        ...
        for (i = 0; i < len; i++){
            str += '<li class="items" dataId="' + data[i]["natId"] + '"><span class="itemValue">' + data[i]["natName"] + '</span><span class="btns"><span class="extra_btn edit_btn"></span><span class="extra_btn delete_btn"></span></span></li>'
        }
    ...
    function deleteItem(event){
        var to_send = {
        	natId: parseInt($(this).parent().parent().attr('dataId'))
        };
        $.ajax({
            url: realPath + '/dictionary/natureDelete',//路径待更正
            type: 'post',
