...
$(function(){
    ...
    function initWeb() {
        ...
        var TableInit = function () {
            ...
            function deleteItem(row) {
                ...
                $.ajax({
                    url: realPath + '/dictionary/categoryDelete',//路径待更正
                    type: 'post',
                    dataType: 'json',
                    async: true,
            ...
            function displaySublist(row){
                ...
                for (i = 0; i < len; i++){
                    node.append('<tr>' +
                        '<td class="belong_id">'+ sublist[i].levId +'</td>' +
                        '<td class="belong_name">'+ sublist[i].levName +'</td>' +
                        '<td>' +
                ...
                $('#sublist').on('click', '.sub-add', function(event){
                    ...
                    function addSave() {
                        var to_send = {
                            catId: row_id,
                            levName: frame.find('.add_value').val()
                        };

                        $.ajax({
                            url: realPath + '/dictionary/categorySubInsert',
                            type: 'post',
                            dataType: 'json',
                ...
                node.on('click', '.belong_update', function(event){
                        ...
                        function updateSave() {
                            var to_send = {
                                levId: update_id,
                                levName: frame.find('.update_value').val()
                            };

                            $.ajax({
                                url: realPath + '/dictionary/categorySubUpdate',
                                type: 'post',
                                dataType: 'json',
                                contentType: 'application/JSON;charset',
                                data: JSON.stringify(to_send),
                    ...
                    .on('click', '.belong_delete', function(event){
                        ...
                        var to_send = {
                            levId: $(event.currentTarget).parents('tr').find('.belong_id').text()
                        };
                        $.ajax({
                            url: realPath + '/dictionary/categorySubDelete',//路径待更正
                            type: 'post',
                            dataType: 'json',
                            async: true,
                            contentType: 'application/JSON;charset=utf-8',
            ...
            oTableInit.Init = function () {
                $('#list').bootstrapTable({
                    url: realPath + '/dictionary/categorySearch',         // 请求后台的URL（*）
                    method: 'post',                      // 请求方式（*）
                    ...
                    columns: [{
                        ...
                        formatter: function (value, row, index) {
                            var delete_btn = "<button disabled class=\"btn btn-xs btn-danger delete_btn\" title=\"删除数据\"><i class=\"glyphicon glyphicon-trash\"></i></button>",
                    ...
                    onEditableSave: function (field, row, oldValue, $el) {
                        $.ajax({
                            type: "post",
                            url: realPath + '/dictionary/categoryUpdate',
                            data: JSON.stringify({
                                catId: row.id,
                                catName: [row.name]
                            }),
                    ...
                    responseHandler: function (data) {
                        var category = data[0]['category'],//键名待更正
                            len = category.length,
                            i;
                        data.total = 0;
                        data.rows = [];
                        ...
                        for (i = 0; i < len; i++) {
                            data.rows.push({
                                ...
                                'levelList': category[i].levelList
                            });
                            data.total += 1
                        }
                        return data;
        ...
        $('#add').on('click', function () {
            ...
            function addSave() {
                ...
                $.ajax({
                    url: realPath + '/dictionary/insertCategorys',
                    type: 'post',
                    dataType: 'json',
                    contentType: 'application/JSON;charset',
                    data: JSON.stringify(to_send),
    ...
    function listRequest(){
        $.ajax({
            url: realPath + '/dictionary/categorySearch',
            type: 'post',
            dataType: 'json',
            async: true,
            success: function(data){
                listData = data[0]['category'];
    ...
    function listDisplay(data){
        ...
        for (i = 0; i < len; i++){
            str += '<li class="items datas" dataId="' + data[i]["catId"] + '"><span class="itemValue">' + data[i]["catName"] + '</span><span class="btns"><span class="extra_btn edit_btn"></span><span class="extra_btn delete_btn"></span></span></li>';
        }
    ...
    function subListDisplay(event){
        ...
        for (i = 0; i < len; i++){
            if (listData[i]['catId'] == catId){//键名待定
                var subArray = listData[i]['levelList'];//键名待定
                this_catId = catId;
                len = subArray.length;
                break
            }
        ...
        for (i = 0; i < len; i++){
            str += '<li class="items" dataId="' + subArray[i]["levId"] + '"><span class="itemValue">' + subArray[i]["levName"] + '</span><span class="btns"><span class="extra_btn edit_btn"></span><span class="extra_btn delete_btn"></span></span></li>';
        }
        str += '<li class="items addParent"><span class="add_btn"></span></li>';
        subList.html(str)
    }
    ...
    function deleteItem(event){
        ...
        var to_send = {
            catId: parseInt($(this).parent().parent().attr('dataId'))
        };
        $.ajax({
            url: realPath + '/dictionary/categoryDelete',//路径待更正
            type: 'post',
    ...
    function deleteSubItem(event){
        var to_send = {
        		...
        		levId: parseInt($(this).parent().parent().attr('dataId'))
        };
        $.ajax({
            url: realPath + '/dictionary/categorySubDelete',//路径待更正
            type: 'post',
