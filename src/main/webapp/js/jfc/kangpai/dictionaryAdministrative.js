...
$(function(){
    ...
    function initWeb(){
        ...
        var TableInit = function() {
            ...
            function deleteItem(row){
                ...
                $.ajax({
                    url: realPath + '/dictionary/administrativeDelete',//路径待更正
                    type: 'post',
                    dataType: 'json',
                    async: true,
            ...
            oTableInit.Init = function () {
                $('#list').bootstrapTable({
                    url: realPath + '/dictionary/administrativeSearch',         // 请求后台的URL（*）
                    method: 'post',                      // 请求方式（*）
                    ...
                    onEditableSave: function (field, row, oldValue, $el) {
                        $.ajax({
                            type: "post",
                            url: realPath + '/dictionary/administrativeUpdate',
                            contentType: 'application/json;charset=utf-8',
                            data: JSON.stringify({
                                admiId: row.id,
                                admiName: row.name
                    ...
                    responseHandler: function(data){
                        var administrative = data[0]['administrative'],//键名待更正
                            len = administrative.length,
                            i;
                        data.total = 0;
                        data.rows = [];
                        ...
                        for (i = 0; i < len; i++){
                            data.rows.push({
                                ...
                                'id': administrative[i].admiId,
                                'name': administrative[i].admiName
                            });
                            data.total += 1
                        }
                        return data;
        ...
        $('#add').on('click', function(){
            ...
            function addSave(){
                ...
                $.ajax({
                    url: realPath + '/dictionary/administrativeInsert',
                    type: 'post',
                    dataType: 'json',
    ...
    function listRequest(){
        $.ajax({
            url: realPath + '/dictionary/administrativeSearch',
            type: 'post',
            dataType: 'json',
            async: true,
            success: function(data){
                listData = data[0]['administrative'];
    ...
    function listDisplay(data){
        ...
        for (i = 0; i < len; i++){
            str += '<li class="items datas" dataId="' + data[i]["admiId"] + '"><span class="itemValue">' + data[i]["admiName"] + '</span><span class="btns"><span class="extra_btn edit_btn"></span><span class="extra_btn delete_btn"></span></span></li>';
        }
    ...
    function deleteItem(event){
        var to_send = {
        		admiId: parseInt($(this).parent().parent().attr('dataId'))
        };
        $.ajax({
            url: realPath + '/dictionary/administrativeDelete',//路径待更正
            type: 'post',
