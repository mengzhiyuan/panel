...
$(function(){
    ...
    function initWeb() {
        ...
        var TableInit = function () {
            ...
            function deleteItem(row) {
                ...
                $.ajax({
                    url: realPath + '/dictionary/specDelete',//路径待更正
                    type: 'post',
                    dataType: 'json',
                    async: true,
            ...
            oTableInit.Init = function () {
                $('#list').bootstrapTable({
                    url: realPath + '/dictionary/specSearch',         // 请求后台的URL（*）
                    method: 'post',                      // 请求方式（*）
                    ...
                    onEditableSave: function (field, row, oldValue, $el) {
                        $.ajax({
                            type: "post",
                            url: realPath + '/dictionary/specUpdate',
                            contentType: 'application/json;charset=utf-8',
                            data: JSON.stringify({
                                specId: row.id,
                                specName: [row.name]
        ...
        $('#add').on('click', function () {
            ...
            function addSave() {
                ...
                $.ajax({
                    url: realPath + '/dictionary/specInsert',
                    type: 'post',
                    dataType: 'json',
    ...
    function listRequest(){
        $.ajax({
            url: realPath + '/dictionary/specSearch',
            type: 'post',
            dataType: 'json',
            async: true,
            success: function(data){
                displayList(data[0]['spec'])
            },
            error: function(d1,d2,d3){
                console.log(d1);
                console.log(d2);
    ...
    function displayList(data){
        ...
        for (i = 0; i < len; i++){
            str += '<li class="items" dataId="' + data[i]["specId"] + '"><span class="itemValue">' + data[i]["specName"] + '</span><span class="btns"><span class="extra_btn edit_btn"></span><span class="extra_btn delete_btn"></span></span></li>'
        }
    ...
    function deleteItem(event){
        var to_send = {
        		specId: parseInt($(this).parent().parent().attr('dataId'))
        };
        $.ajax({
            url: realPath + '/dictionary/specDelete',//路径待更正
            type: 'post',
