/**
 *
 * Created by wangxiangyang on 2018/5/4.
 */
...
const methods = {
    switchBoard: function(event){
        if (!$(event.target).attr('target')) return;
        this.status.process = $(event.target).attr('target');
        if (this.status.process === 'update'){
            setTimeout(function(){
                chainReact();
                laydate.render({
                    elem: '#age_to_update',
    ...
    assignmentPrecise: function(opt){
    ...
    assign: function(row, field){
        if (row[field]){
            switch (field){
                case 'nameComp':
                    this.to_update['hosId'] = row['hosId'];
                    this.to_update['nameComp'] = row['nameComp'];
                    break;
                case 'sex':
    ...
    assignmentQuick: function(id){
        this.to_insert.hosId = id
    },
...
const mounted = function(){
    ...
    laydate.render({
        elem: '#new_hospital_found',
        type: 'date',
        istime: true,
        done: function(value, date){
            self.to_insert.foundTime = value;
            //console.log(value);
            //console.log(date);
        }
    });
